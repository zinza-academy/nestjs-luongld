...
export class NewMigration1723697823025 implements MigrationInterface {
  name = 'NewMigration1723697823025';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`district\` (\`id\` int NOT NULL, \`district\` varchar(255) NOT NULL, \`provinceId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`province\` (\`id\` int NOT NULL, \`province\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`ward\` (\`id\` int NOT NULL, \`ward\` varchar(255) NOT NULL, \`districtId\` int NULL, \`provinceId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
